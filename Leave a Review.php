<?php
    ...
    if (isset($_POST["submit"])) {
      ...
      if($star!="" && !empty($rTitle) && !empty($rDez) && $cmid!=0) {
        header("HTTP/1.1 307 Temprary Redirect"); 
        header("Location: leave2.php");
      } else {
        echo '<script type="text/javascript">','alert("Please enter information in all fields.\\nNote: A star Rating cannot be 0.");','</script>';
      }
    } 
?>
<!DOCTYPE html>
<html>
    ...
    <script>
      ...
      function backFunction(){
        document.location.href = "exploreMeals.php";
      }
      function error(){
        var message = alert("Please enter information in all fields. \
        Note: A star Rating cannot be 0. \ ");
      }
    </script>
    <body>
    ...
    <div id="title2"><h1 id="title">Leave a Review</h1></div>
    <div id="body"> 
    ...
    <select name="revMeal" id="input">
      <option value=0>Select Meal</option>
      <?php  
        require_once("db.php");
        
        $sql = "SELECT CMID, meal FROM mealchoices";
        
        $result=$mydb->query($sql);
        while($row = mysqli_fetch_array($result)){
            echo "<option value='".$row["CMID"]."'>".$row["meal"]."</option>";
        }
      ?>
    </select>
    <br>
    ...
    <p>
    <label>Review Title
        <input type="text" name="reviewTitle" size="25" id="meal" autofocus required> 
    </label>
    </p>
    <p>
      <label>Review Description 
      <textarea name="reviewDescription" rows="4" cols="50" id="description"></textarea>
    </label>
    </p>  
    </section>
